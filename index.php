...
function request($url,$type='get',$param=[]){
	...
	if( $type=='post' ){
		...
		$headers_login = [
			...
			"Cookie:token=ea8d1a2b2e637c665e6640b8bc4619cc; userId=10678; PHPSESSID=g372ektj3rqa7optb4j6jb0abf; isLogin=1; Hm_lvt_02f32149c7ea90d0cd47ed89025e457c=1518315926,1519368479,1519374574,1519525930; Hm_lpvt_02f32149c7ea90d0cd47ed89025e457c=".time(),
		];
    ...
    public function get_questions($url){
        ...
        if( preg_match_all($reg1,$text,$res1) ){
            foreach( $res1[1] as $v1 ){
                ...
                $sql = 'insert into question set version="'.$this ->version.'",question="'.trim($question).'",answer="'.$answer.'",analysis="'.$analysis.'",type="'.$this ->sub_type.'",level="'.$this ->sub_level.'",name="'.$this ->sub_name.'",url="'.$url.'"';
